(() => {
    ...
    const leagueButtons = leagues.length
      ? leagues.map(l => `
          <button class="league-option ${selected?.league_id === l.league_id ? 'active' : ''}"
                  ...
                  type="button"
                  onclick="window.OFB_SELECT_LEAGUE_FROM_BTN && window.OFB_SELECT_LEAGUE_FROM_BTN(this)">
    ...
    els.teamCardBox.innerHTML = `
      <article class="card">
        ...
        <button class="primary-button full-width subscribe-line"
                ...
                type="button"
                onclick="window.OFB_SUBSCRIBE_SELECTED_TEAM && window.OFB_SUBSCRIBE_SELECTED_TEAM('inline_click')"
  ...
  function bindSubscribeButtonTap() {
    const btn = document.getElementById('subscribeSelectedLeagueBtn');
    if (!btn || btn.dataset.tapBound === '1') return;

    btn.dataset.tapBound = '1';

    // Обычный click оставляем для браузера/ПК. В MAX WebView на телефоне
    // иногда click по динамической кнопке не доходит, поэтому ниже есть
    // отдельный pointerdown/touchstart только для этой кнопки.
    btn.addEventListener('click', (e) => {
      e.preventDefault();
      e.stopPropagation();
      runSubscribeButton('button_click', e, btn);
    });
  }
  ...
  function bindEvents() {
    ...
    document.addEventListener('click', (e) => {
      ...
      if (e.target.closest('#subscribeSelectedLeagueBtn')) {
        subscribeSelectedTeam('delegated_click');
        return;
      }
  ...
  function bindMobileSheetCaptureEvents() {
    if (window.__OFB_MOBILE_CAPTURE_BOUND__) return;
    window.__OFB_MOBILE_CAPTURE_BOUND__ = true;

    const handledRecently = { subscribe: 0, league: 0 };

    const handle = (e) => {
      const target = e.target;
      if (!target?.closest) return;

      const subscribeBtn = target.closest('#subscribeSelectedLeagueBtn');
      if (subscribeBtn) {
        const now = Date.now();
        if (now - handledRecently.subscribe < 700) return;
        handledRecently.subscribe = now;

        if (e.cancelable) e.preventDefault();
        e.stopPropagation();
        runSubscribeButton(`capture_${e.type}`, e, subscribeBtn);
        return;
      }

      const leagueBtn = target.closest('[data-select-league]');
      if (leagueBtn) {
        const now = Date.now();
        if (now - handledRecently.league < 300) return;
        handledRecently.league = now;

        const leagueId = leagueBtn.dataset.selectLeague;
        renderDebug({ leagueTapSource: `capture_${e.type}`, leagueId });
        selectLeague(leagueId);
      }
    };

    document.addEventListener('pointerdown', (e) => {
      if (e.pointerType === 'touch') handle(e);
    }, { capture: true, passive: false });

    document.addEventListener('touchstart', handle, { capture: true, passive: false });
  }

  window.OFB_SELECT_LEAGUE_FROM_BTN = function(btn) {
    if (!btn) return;
    selectLeague(btn.dataset.selectLeague);
  };

  window.OFB_SUBSCRIBE_SELECTED_TEAM = function(source = 'window_inline') {
    const btn = document.getElementById('subscribeSelectedLeagueBtn');
    runSubscribeButton(source, null, btn);
  };
