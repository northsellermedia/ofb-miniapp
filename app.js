(() => {
  ...
  const CACHE_KEY = 'ofb_mini_app_v15_cache';
  ...
  async function apiAction(action, payload = {}, options = {}) {
    ...
    if (options.abortPreviousSearch) state.searchRequest = request;
  ...
  async function searchTeams() {
    ...
    if (els.searchHint) els.searchHint.textContent = 'Ищем в базе OFB…';
    if (els.searchResults) {
      els.searchResults.innerHTML = '<div class="item"><div class="item-main"><div class="item-title">Поиск…</div><div class="item-meta">Ищем команду сразу с турниром.</div></div></div>';
    }

    renderTeamSelection(null);

    try {
      const data = await apiAction('search_catalog', { q }, {
        disableUi: false,
        allowParallel: true,
      ...
      if (els.searchHint) els.searchHint.textContent = items.length ? `Найдено: ${items.length}` : 'Команды не найдены';
      if (!els.searchResults) return;

      els.searchResults.innerHTML = items.length
        ? items.map(item => `<div class="item search-result"><div class="item-main"><div class="item-title">${escapeHtml(item.team_name || 'Команда')}</div><div class="item-meta">${escapeHtml(item.league_name || 'Турнир')}</div></div><button class="primary-button compact" type="button" data-select-catalog="1" data-team-id="${escapeAttr(item.team_id)}" data-team-name="${escapeAttr(item.team_name || '')}" data-league-id="${escapeAttr(item.league_id)}" data-league-name="${escapeAttr(item.league_name || '')}">Выбрать</button></div>`).join('')
        : '<div class="item"><div class="item-main"><div class="item-title">Ничего не найдено</div><div class="item-meta">Попробуйте другое написание: Milan, AC Milan, Inter, Real Madrid.</div></div></div>';
    } catch (e) {
  ...
  function selectCatalogItem(teamId, teamName, leagueId, leagueName) {
    if (!teamId || !leagueId) return;

    state.selectedTeam = {
      team_id: Number(teamId),
      team_name: teamName || `Команда ${teamId}`
    };

    state.selectedLeague = {
      league_id: Number(leagueId),
      league_name: leagueName || 'Турнир'
    };

    renderTeamSelection({
      team: state.selectedTeam,
      leagues: [state.selectedLeague],
  ...
  function renderTeamSelection(data) {
    ...
    els.teamCardBox.innerHTML = `<div class="team-card"><div class="team-card-head"><img class="team-card-logo" src="https://media.api-sports.io/football/teams/${escapeAttr(team.team_id)}.png" alt="" onerror="this.src='./assets/ofb-logo.jpg'"><div><h3>${escapeHtml(team.team_name)}</h3><p class="small-muted">Проверьте команду и турнир</p></div></div><div class="league-list">${leagues.map(l => `<button class="league-option ${selected?.league_id == l.league_id ? 'active' : ''}" type="button" data-select-league="${escapeAttr(l.league_id)}" data-league-name="${escapeAttr(l.league_name)}">${escapeHtml(l.league_name)}${l.matches_count ? `<small>${escapeHtml(l.matches_count)} матч.</small>` : ''}</button>`).join('')}</div><button class="primary-button full-width" type="button" id="confirmSubscriptionBtn" ${canConfirm ? '' : 'disabled'}>${canConfirm ? 'Подтвердить выбор' : 'Выберите вариант из списка'}</button></div>`;
  }
  ...
  async function confirmSubscription() {
    if (!state.selectedTeam?.team_id || !state.selectedLeague?.league_id) { showToast('Выберите команду'); return; }
    try {
  ...
  function bindEvents() {
    ...
    document.addEventListener('click', e => {
      const catalogBtn = e.target.closest('[data-select-catalog]');
      if (catalogBtn) {
        const key = 'catalog-' + catalogBtn.dataset.teamId + '-' + catalogBtn.dataset.leagueId;
        if (!dedupe(key)) return;
        selectCatalogItem(
          catalogBtn.dataset.teamId,
          catalogBtn.dataset.teamName,
          catalogBtn.dataset.leagueId,
          catalogBtn.dataset.leagueName
        );
        return;
      }
