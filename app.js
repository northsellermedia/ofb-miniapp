(() => {
  ...
  function getUrlParamFromHashOrSearch(...names) {
  ...
  function getMaxInitData() {
    ...
    const fromUrl = getUrlParamFromHashOrSearch('WebAppData', 'initData', 'webAppData', 'web_app_data');
  ...
  function unwrapApiData(data) {
    ...
    if (parsed.response && typeof parsed.response === 'object' && (parsed.response.ok !== undefined || parsed.response.team || parsed.response.teams)) {
      return parsed.response;
    }

    if (parsed.data && typeof parsed.data === 'object' && (parsed.data.ok !== undefined || parsed.data.team || parsed.data.teams)) {
      return parsed.data;
    }
  ...
  async function searchTeams() {
    ...
    try {
      ...
      const teams = arrayFromAny(data.teams, data.items, data.results);
  ...
  async function openTeamCard(teamId, fallbackTeamName = '', sourceButton = null) {
    ...
    try {
      ...
      let availableLeagues = getLeaguesFromTeamCard(data);
      const selectedLeague = getSelectedLeagueFromTeamCard(data, availableLeagues);
  ...
  function normalizeLeague(raw) {
    ...
    const nestedLeague = safeObject(obj.league);
    const nestedCountry = safeObject(obj.country);
    const leagueId = Number(obj.league_id || obj.id || nestedLeague.id);
    const leagueName = String(obj.league_name || obj.name || nestedLeague.name || '').trim();

    if (!Number.isFinite(leagueId) || leagueId <= 0 || !leagueName) return null;

    return {
      ...obj,
      league_id: leagueId,
      league_name: leagueName,
      country: obj.country_name || obj.country || nestedCountry.name || '',
      season: obj.season || obj.year || '',
      matches_count: obj.matches_count ?? obj.count ?? null,
      first_match: obj.first_match || obj.firstFixture || '',
      last_match: obj.last_match || obj.lastFixture || ''
    };
  }

  function getLeaguesFromTeamCard(data) {
    const team = safeObject(data?.team);
    const profile = safeObject(data?.profile);
    const leagues = arrayFromAny(
      data?.available_leagues,
      data?.leagues,
      data?.availableLeagues,
      data?.team_leagues,
      data?.teamLeagues,
      team.available_leagues,
      team.leagues,
      profile.available_leagues,
      profile.leagues
    )
      .map(normalizeLeague)
      .filter(Boolean);
    ...
    return leagues.filter((league) => {
      const key = `${league.league_id}:${league.league_name}`;
  ...
  function getSelectedLeagueFromTeamCard(data, availableLeagues) {
    const selected = normalizeLeague(
      data?.selected_league ||
      data?.selectedLeague ||
      data?.team?.selected_league ||
      data?.team?.selectedLeague ||
      data?.team
    );

    if (selected?.league_id && selected?.league_name) return selected;
    if (availableLeagues.length === 1) return availableLeagues[0];
    return null;
  }
  ...
  function bindTeamCardInteractiveTaps() {
    ...
    document.querySelectorAll('[data-select-league]').forEach((btn) => {
      ...
      btn.addEventListener('touchend', (e) => { e.preventDefault(); handleLeague('league_touchend', e); }, { passive: false });
      btn.addEventListener('pointerup', (e) => {
        if (e.pointerType === 'touch' || e.pointerType === 'pen') { e.preventDefault(); handleLeague('league_pointerup', e); }
      }, { passive: false });
    });
    ...
    if (subscribeBtn && subscribeBtn.dataset.tapBound !== '1') {
      ...
      subscribeBtn.addEventListener('touchend', (e) => { e.preventDefault(); handleSubscribe('subscribe_touchend', e); }, { passive: false });
      subscribeBtn.addEventListener('pointerup', (e) => {
        if (e.pointerType === 'touch' || e.pointerType === 'pen') { e.preventDefault(); handleSubscribe('subscribe_pointerup', e); }
      }, { passive: false });
    }
  }
  ...
  function bindEvents() {
    ...
    document.addEventListener('pointerup', (e) => {
      ...
      if (teamCardBtn && (e.pointerType === 'touch' || e.pointerType === 'pen')) {
        e.preventDefault();
        teamCardBtn.dataset.openTeamCardPointerup = String(Date.now());
    ...
    document.addEventListener('click', (e) => {
      const teamCardBtn = e.target.closest('[data-open-team-card]');
      if (teamCardBtn) {
        const lastPointer = Number(teamCardBtn.dataset.openTeamCardPointerup || 0);
        if (lastPointer && Date.now() - lastPointer < 900) return;
        openTeamCard(teamCardBtn.dataset.openTeamCard, teamCardBtn.dataset.teamName, teamCardBtn);
        return;
      }
  ...
  // V10.15: глобальный capture touch убран. Touch/tap обработчики вешаются только на реальные кнопки после рендера карточки.
