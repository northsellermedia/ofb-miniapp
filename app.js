(() => {
  ...
  function renderStats(stat) {
    // В текущем index.html нет старого блока statsSeason/statsEmpty/statPlayed...
    // Раньше тут был краш на null-элементах, из-за чего после матчей не рендерились:
    // сезон, состав и трансферы.
  ...
  async function searchTeams() {
    ...
    try {
      const data = await ofbApi('/ofb-api-team-search', { q }, { noCache: true, timeoutMs: 20000, retries: 1, saveLocal: false });
      const teams = safeArr(data.teams);
      els.searchHint.textContent = teams.length ? 'Выберите команду, потом лигу.' : 'Команды не найдены.';
  ...
  async function openTeamCard(teamId, fallbackTeamName = '') {
    state.selectedTeamCard = null;
    state.selectedLeague = null;
    els.searchHint.textContent = 'Загружаем карточку команды…';
    renderTeamCard({
      loading: true,
      team: {
    ...
    try {
      ...
      els.searchHint.textContent = availableLeagues.length > 1
        ? 'Выберите лигу/турнир для подписки.'
        : 'Лига выбрана автоматически.';
    ...
    els.teamCardBox.innerHTML = `
      <article class="card">
        <div class="item">
          ...
          <div class="item-main">
            <div class="item-title">${escapeHtml(team.team_name || 'Команда')}</div>
            <div class="item-meta">${isSubscribed ? 'Команда уже выбрана. Можно сменить турнир.' : 'Выберите турнир для подписки.'}</div>
          </div>
        </div>
  ...
  function bindEvents() {
    ...
    document.addEventListener('click', (e) => {
      ...
      if (teamCardBtn) {
        openTeamCard(teamCardBtn.dataset.openTeamCard, teamCardBtn.dataset.teamName);
        return;
      }
