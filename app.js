(() => {
  ...
  async function openApp(force = false) {
    ...
    try {
      ...
      const cached = loadCache();
      if (cached?.screen === 'home') { renderHome(cached, true); showToast('Показали последние загруженные данные'); return; }
      setVisible(els.loadingState, false);
  ...
  async function searchTeams() {
    ...
    try {
      ...
      els.searchResults.innerHTML = teams.length ? teams.map(team => `<div class="item search-result"><div class="item-main"><div class="item-title">${escapeHtml(team.team_name || 'Команда')}</div><div class="item-meta">Нажмите, чтобы выбрать команду</div></div><button class="primary-button compact" type="button" data-select-team="${escapeAttr(team.team_id)}" data-team-name="${escapeAttr(team.team_name || '')}">Выбрать</button></div>`).join('') : '<div class="item"><div class="item-main"><div class="item-title">Ничего не найдено</div><div class="item-meta">Попробуйте название на английском.</div></div></div>';
    } catch (e) {
  ...
  function bindEvents() {
    ...
    els.clearAppCacheBtn?.addEventListener('click', () => { clearCache(); showToast('Данные обновлены'); openApp(true); });
