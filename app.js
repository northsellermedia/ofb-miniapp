(() => {
  ...
  function extractChangeLockInfo(data = {}) {
    const src = data?.response || data?.subscription || data || {};
    const resultStatus = src.result_status || src.status || data?.result_status || '';
    const errorCode = src.error || data?.error || '';
    const allowed = src.allowed ?? data?.allowed;
    const locked = allowed === false || allowed === 'false' || resultStatus === 'change_locked' || errorCode === 'change_locked' || errorCode === 'team_change_locked';
    if (!locked) return null;

    return {
      nextAllowedAt: src.next_allowed_at || src.nextAllowedAt || data?.next_allowed_at || data?.nextAllowedAt || '',
      teamName: src.team_name || data?.team_name || state.currentTeam?.team_name || 'текущая команда'
    };
  }

  function showChangeLockedNotice(lockInfo = {}) {
    const date = formatLongDate(lockInfo.nextAllowedAt);
    const text = date
      ? `Смена команды пока недоступна. Следующая смена: ${date}.`
      : `Смена команды пока недоступна. ${teamChangeWarningText()}`;

    state.changeLock = {
      nextAllowedAt: lockInfo.nextAllowedAt || '',
      teamName: lockInfo.teamName || state.currentTeam?.team_name || 'текущая команда'
    };

    if (els.searchHint) els.searchHint.textContent = text;
  ...
  async function openTeamCard(teamId, fallbackTeamName = '', sourceButton = null) {
    ...
    try {
      ...
      const availableLeagues = safeArr(data.available_leagues);
  ...
  function renderTeamCard(card) {
    ...
    const disabled = !selected?.league_id || !selected?.league_name || state.isSubscribing;
    const buttonText = state.isSubscribing
      ? 'Проверяем возможность смены…'
      : (!selected?.league_id || !selected?.league_name)
        ? 'Сначала выберите лигу'
        : `${isSubscribed ? 'Обновить подписку' : 'Подписаться'} на ${team.team_name} / ${selected.league_name}`;

    const lockDate = state.changeLock?.nextAllowedAt ? formatLongDate(state.changeLock.nextAllowedAt) : '';
    const lockHtml = state.changeLock ? `
        <div class="warning-box locked-box">
          ⏳ Смена команды пока недоступна.<br />
          ${state.changeLock.teamName ? `Текущая команда: <b>${escapeHtml(state.changeLock.teamName)}</b><br />` : ''}
          ${lockDate ? `Следующая смена: <b>${escapeHtml(lockDate)}</b>.` : escapeHtml(teamChangeWarningText())}
        </div>
    ` : '';
  ...
  async function subscribeSelectedTeam() {
    ...
    const confirmed = window.confirm(
      `Выбрать основную команду ${team.team_name} / ${league.league_name}?

` +
      `После сохранения команду можно будет сменить только через ${TEAM_CHANGE_LOCK_DAYS} дней.`
    );
    ...
    setButtonBusy(subscribeBtn, true, 'Сохраняем…');
    ...
    try {
      ...
      const lockInfo = extractChangeLockInfo(result);
      if (lockInfo) {
        showChangeLockedNotice(lockInfo);
        return;
      }
      ...
      const lockInfo = extractChangeLockInfo(e.data || { error: errorCode, next_allowed_at: nextAllowedAt });
      if (lockInfo) {
        showChangeLockedNotice(lockInfo);
        return;
      }
      ...
      if (els.searchSheet && !els.searchSheet.classList.contains('hidden')) {
        renderTeamCard(state.selectedTeamCard);
      }
    }
  }
