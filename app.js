(() => {
  ...
  const STORAGE_PREFIX = 'ofb_mini_app_v4:';
  ...
  function showToast(text) {
    ...
    showToast._t = window.setTimeout(() => setVisible(els.toast, false), 2200);
  }
  ...
  function normalizeCurrentTeamFromDashboard(dashboard) {
    ...
    const sub = safeArr(dashboard?.subscriptions)[0] || null;
    if (!sub) return null;
    ...
    return {
      ...
      logo_url: dashboard?.profile?.logo_url || ''
    };
  }
  ...
  function renderInitialPlaceholders() {
    if (els.nextMatches && !els.nextMatches.children.length) {
      renderBlockLoading(els.nextMatches, 'Подгружаем ближайшие матчи');
    }

    if (els.matchesList && !els.matchesList.children.length) {
      renderBlockLoading(els.matchesList, 'Откройте вкладку, матчи подгрузятся');
    }

    if (els.statsEmpty) {
      els.statsEmpty.textContent = 'Статистика сезона берётся из dashboard.';
  ...
  function renderDashboard(dashboard, fromCache = false) {
    ...
    renderFixtures(safeArr(dashboard.fixtures), fromCache);
    renderStats(safeArr(dashboard.stats)[0] || null);
  ...
  async function loadDashboard() {
    ...
    const localDashboard = loadLocal('/ofb-api-dashboard', {}, 6 * 60 * 60 * 1000);
    let hadLocal = false;
    ...
    try {
      const dashboard = await ofbApi('/ofb-api-dashboard', {}, {
        noCache: true,
        timeoutMs: 20000,
        retries: 1
      });
      ...
      if (!hadLocal) {
        ...
        renderBlockError(els.nextMatches, 'Dashboard не загрузился', e.message);
        showToast('Dashboard не загрузился');
      } else {
        console.warn('Dashboard не обновился, показываем сохранённый', e);
      }
    }
  }
  ...
  async function loadSquad() {
    ...
    const local = loadLocal('/ofb-api-team-squad', payload);
    const hadLocal = !!local;

    if (local) renderSquad(local, true);
    else renderBlockLoading(els.squadGrid, 'Загружаем состав');
    ...
    try {
      ...
      if (!hadLocal) {
        ...
        console.warn('Состав не обновился, показываем сохранённый', e);
      }
    }
  }
  ...
  async function loadTransfers() {
    ...
    const payload = { team_id: state.currentTeam.team_id };
    const local = loadLocal('/ofb-api-team-transfers', payload);
    const hadLocal = !!local;

    if (local) renderTransfers(local, true);
    else renderBlockLoading(els.transfersList, 'Загружаем трансферы');
    ...
    try {
      ...
      if (!hadLocal) {
        ...
        console.warn('Трансферы не обновились, показываем сохранённые', e);
      }
    }
  }
  ...
  async function updateFlags(changedFlag, checked) {
    ...
    try {
      ...
      const cached = loadLocal('/ofb-api-dashboard', {}, 6 * 60 * 60 * 1000);
      if (cached?.team) {
        cached.team.notify_flags = { ...state.flags };
        saveLocal('/ofb-api-dashboard', {}, cached);
      }
