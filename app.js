...
function teamLogoUrl(team = {}) {
  const explicit = team.logo_url || team.logo || team.team_logo || team.photo_url;
  if (explicit) return explicit;

  const id = Number(team.team_id || team.id);
  if (Number.isFinite(id) && id > 0) {
    // CDN-картинка клуба. Это не API-запрос, ключи не нужны.
    return `https://media.api-sports.io/football/teams/${id}.png`;
  }

  return 'assets/logo-ofb.jpg';
}

function renderTeamMiniLogo(team = {}) {
  return `
    <span class="team-mini-logo-wrap">
      <img
        class="team-mini-logo"
        src="${escapeHtml(teamLogoUrl(team))}"
        alt="${escapeHtml(team.team_name || 'Команда')}"
        loading="lazy"
...
function renderSubscriptions() {
  ...
  els.subscriptionsList.innerHTML = state.subscriptions.map((team) => `
    <article class="item-card team-list-card">
      <div class="item-card-row">
        <div class="team-list-left">
          ${renderTeamMiniLogo(team)}
        ...
        <button class="soft-btn open-team-btn" type="button" data-open-team="${team.team_id}">Открыть</button>
      </div>
    </article>
  `).join('');
}
  ...
  els.searchResults.innerHTML = state.searchResults.map((team) => `
    <article class="item-card team-list-card">
      <div class="item-card-row">
        ...
        <button class="primary-btn open-team-btn" type="button" data-open-team="${team.team_id}">Открыть</button>
      </div>
    </article>
  `).join('');
}
  ...
  els.teamCard.innerHTML = `
    <section class="team-title-card glass-panel">
      ...
      <div class="team-actions">
        ...
        <button class="soft-btn" type="button" data-team-tab="notify">Уведомления</button>
      </div>
    </section>
...
function renderTeamLogo() {
  ...
  const logo = profile.logo_url || profile.logo || teamLogoUrl(state.selectedTeam?.team || {});
  return `
    <img
      class="team-logo"
      src="${escapeHtml(logo)}"
