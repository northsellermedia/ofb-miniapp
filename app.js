(() => {
  ...
  const CACHE_KEY = 'ofb_mini_app_v12_cache';
  ...
  async function apiAction(action, payload = {}, options = {}) {
    ...
    if (state.activeRequest && !options.allowParallel) {
      showToast('Подождите, запрос уже выполняется');
      return null;
    }
    ...
    try {
      ...
      if (e.name === 'AbortError') throw new Error('Запрос слишком долго выполнялся. Попробуйте ещё раз.');
      throw e;
    } finally {
      clearTimeout(timer);
  ...
  function renderSquad(squad) {
    ...
    if (!list.length) { els.squadGrid.innerHTML = '<div class="item"><div class="item-main"><div class="item-title">Состав пока не найден</div><div class="item-meta">Кэш обновляется по расписанию.</div></div></div>'; return; }
  ...
  async function openApp(force = false) {
    ...
    try {
      ...
      const cached = loadCache();
      if (cached?.screen === 'home') { renderHome(cached, true); showToast('Показали сохранённые данные'); return; }
      setVisible(els.loadingState, false);
  ...
  async function searchTeams() {
    ...
    if (els.searchResults) els.searchResults.innerHTML = '<div class="item"><div class="item-main"><div class="item-title">Поиск…</div></div></div>';
    renderTeamSelection(null);
    try {
      const data = await apiAction('search_team', { q }, { disableUi: false });
      ...
      els.searchResults.innerHTML = teams.length ? teams.map(team => `<div class="item search-result"><div class="item-main"><div class="item-title">${escapeHtml(team.team_name || 'Команда')}</div><div class="item-meta">ID ${escapeHtml(team.team_id)}</div></div><button class="primary-button compact" type="button" data-select-team="${escapeAttr(team.team_id)}" data-team-name="${escapeAttr(team.team_name || '')}">Выбрать</button></div>`).join('') : '<div class="item"><div class="item-main"><div class="item-title">Ничего не найдено</div><div class="item-meta">Попробуйте название на английском.</div></div></div>';
    } catch (e) {
  ...
  function bindEvents() {
    ...
    els.clearAppCacheBtn?.addEventListener('click', () => { clearCache(); showToast('Кэш очищен'); openApp(true); });
  ...
  async function init() {
    ...
    if (!state.initData) {
      setVisible(els.loadingState, false);
      setVisible(els.authNotice, true);
      renderNoTeam('Открой mini app внутри MAX. Без initData приложение не сможет проверить вход.');
      return;
    }
    await openApp(false);
  }
