(() => {
  ...
  function handleInteractiveActionFromEvent(e, source = 'event') {
    ...
    if (subscribeBtn) {
      ...
      e.stopPropagation();
      renderDebug({ subscribeTouchSource: source, subscribeReady: subscribeBtn.dataset.subscribeReady || null });

      if (subscribeBtn.dataset.subscribeReady === '0') {
        showToast('Сначала выберите лигу');
    ...
    const teamCardBtn = target.closest('[data-open-team-card]');
    if (teamCardBtn) {
      if (e.cancelable) e.preventDefault();
      e.stopPropagation();
  ...
  function bindEvents() {
    ...
    document.addEventListener('touchend', (e) => {
      const handled = handleInteractiveActionFromEvent(e, 'touchend');
      if (handled) state.lastTouchActionAt = Date.now();
    }, { passive: false, capture: true });

    document.addEventListener('pointerup', (e) => {
      if (e.pointerType !== 'touch') return;
      const handled = handleInteractiveActionFromEvent(e, 'pointerup');
      if (handled) state.lastTouchActionAt = Date.now();
    }, { passive: false, capture: true });

    document.addEventListener('click', (e) => {
      if (Date.now() - (state.lastTouchActionAt || 0) < 700) return;
      if (handleInteractiveActionFromEvent(e, 'click')) return;
