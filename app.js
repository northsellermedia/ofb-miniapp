(() => {
  ...
  function setTeamLogoFallback() {
    if (!state.currentTeam?.team_id || !els.teamLogo) return;

    // Быстрый логотип напрямую по ID API-Football.
    // Это не backend API запрос, а обычная картинка CDN.
    const cdnLogo = `https://media.api-sports.io/football/teams/${state.currentTeam.team_id}.png`;

    if (!els.teamLogo.src || els.teamLogo.src.includes('ofb-logo.jpg')) {
      els.teamLogo.src = cdnLogo;
  ...
  function renderInitialPlaceholders() {
    ...
    if (els.statsEmpty) {
      els.statsEmpty.textContent = 'Статистика сезона грузится в фоне и может появиться позже.';
  ...
  async function loadProfile() {
    ...
    try {
      ...
      if (state.currentTeam) {
        ...
        // ВАЖНО: не блокируем главный экран долгими endpoint.
        // Экран показывается сразу, данные догружаются фоном.
        loadFixtures().catch(console.warn);
        loadTeamProfile().catch(console.warn);

        // Сезонная статистика самая тяжёлая/нестабильная, поэтому грузим позже.
        window.clearTimeout(loadProfile._statsTimer);
        loadProfile._statsTimer = window.setTimeout(() => {
    ...
    const render = (data) => {
      ...
      if (profile?.logo_url || profile?.logo) {
        ...
        if (logo && typeof logo === 'string') {
          ...
          els.teamLogo.onerror = () => {
            els.teamLogo.onerror = null;
            els.teamLogo.src = `https://media.api-sports.io/football/teams/${state.currentTeam.team_id}.png`;
          };
          setVisible(els.teamLogo, true);
        }
      }
  ...
  async function loadFixtures() {
    ...
    try {
      ...
      if (!hadLocal) {
        ...
        console.warn('Матчи не обновились, показываем сохранённые', e);
      }
    }
  }
  ...
  async function loadStats() {
    ...
    try {
      ...
      // Не затираем старую статистику пустотой, если она уже была на экране.
      if (els.statPlayed.textContent === '—') {
        renderStats(null);
      }
    }
  }
