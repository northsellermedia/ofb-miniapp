(() => {
  ...
  const CACHE_KEY = 'ofb_mini_app_v16_cache';
  ...
  async function searchTeams() {
    ...
    if (!q) {
      ...
      if (els.searchHint) els.searchHint.textContent = 'Введите название команды и нажмите «Найти».';
      return;
    }
    ...
    try {
      ...
      // Если пользователь уже запустил новый поиск, старый ответ не перерисовывает экран.
      ...
      els.searchResults.innerHTML = items.length
        ? items.map(item => `
          <div class="item search-result">
            <div class="item-main">
              <div class="item-title">${escapeHtml(item.team_name || 'Команда')}</div>
              <div class="item-meta">${escapeHtml(item.league_name || 'Турнир')}</div>
            </div>
            <button
              class="primary-button compact"
              type="button"
              data-select-catalog="1"
              data-team-id="${escapeAttr(item.team_id)}"
              data-team-name="${escapeAttr(item.team_name || '')}"
              data-league-id="${escapeAttr(item.league_id)}"
              data-league-name="${escapeAttr(item.league_name || '')}"
            >Выбрать</button>
          </div>
        `).join('')
