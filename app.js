  ...
  return `
    <section class="hero-card glass-card">
      ...
      <div class="hero-actions">
        ...
        <button class="btn secondary" data-go="teams" type="button">Мои команды · ${teamsCount}</button>
      </div>
    </section>
...
function renderTeamView() {
  ...
  return `
    <section class="hero-card glass-card team-hero">
      ...
      <p class="hero-text">Центр команды: матчи, уведомления и будущие разделы в одной карточке.</p>
      <div class="hero-actions">
        ${isSubscribedToTeam
    ...
    <section class="section">
      <div class="segment-nav" aria-label="Раздел команды">
        ${renderTeamPanelButton('matches', 'Матчи')}
        ${renderTeamPanelButton('notifications', 'Уведомления')}
        ${renderTeamPanelButton('news', 'Новости')}
        ${renderTeamPanelButton('table', 'Таблица')}
        ${renderTeamPanelButton('squad', 'Состав')}
      </div>
      ${renderTeamPanel(state.teamPanel, teamId, isSubscribedToTeam, flags, card)}
    </section>
  `;
}

function renderTeamPanelButton(panel, title) {
  return `<button class="segment-button ${state.teamPanel === panel ? 'active' : ''}" data-team-panel="${panel}" type="button">${escapeHtml(title)}</button>`;
}

function renderTeamPanel(panel, teamId, isSubscribedToTeam, flags, card) {
    ...
    return `
      <div class="section-head compact">
        <div>
          ...
          <p class="section-subtitle">Меняются отдельно для каждой команды.</p>
        </div>
      </div>
  ...
  if (panel === 'news') {
    return renderComingSoon('Новости команды', 'Этот раздел подключим после отдельного backend endpoint для новостей по team_id. Сейчас новости уже могут приходить через уведомления.');
  }

  if (panel === 'table') {
    return renderComingSoon('Турнирная таблица', 'Нужен отдельный cache workflow standings и endpoint чтения из Postgres. API-Football по клику не дергаем.');
  }

  if (panel === 'squad') {
    return renderComingSoon('Состав и игроки', 'Подключим позже через фоновый squad cache. Сейчас этот раздел оставлен в каркасе, чтобы дизайн не переделывать.');
  }
...
function renderComingSoon(title, text) {
  return `
    <div class="empty-card coming-card">
  ...
  return `
    ...
    <section class="section">
      <div class="settings-card">
        ...
        <div class="button-row">
          <button class="btn secondary" id="refreshProfileButton" type="button">Обновить профиль</button>
          <button class="btn secondary" id="openChannelButton" type="button">Открыть канал OFB</button>
        </div>
      </div>
    </section>
  `;
...
function bindViewEvents() {
  ...
  appEl.querySelectorAll('[data-team-panel]').forEach((button) => {
    button.addEventListener('click', () => {
      state.teamPanel = button.dataset.teamPanel || 'matches';
      renderApp();
    });
  });
  ...
  if (refreshProfileButton) refreshProfileButton.addEventListener('click', async () => {
    await loadProfile({ silent: true });
    showToast('Профиль обновлён');
  });

  const openChannelButton = document.getElementById('openChannelButton');
  if (openChannelButton) openChannelButton.addEventListener('click', () => {
    window.open('https://max.ru/ofb24news', '_blank', 'noopener,noreferrer');
  });
}
